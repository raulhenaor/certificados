...
<body>
 ...
 <div class="container">    
    ...
    <div class="contenedorCurso mitad">
        ...
        <div class="contenedorCurso mitad">
             <div class="tituloCurso">
                   ...
                   <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">        
                           ...
                           <div class="row">
                                <div class="col">
                           <label for="exampleInputEmail1" class="form-label">Ingrese el código del Certificado</label>
                           <input type="text" class="form-control" name="buscarPasajero" id="buscarPasajero">
                           <div id="emailHelp" class="form-text">Inserte Certificado.</div>
                                </div>
                               </div>
                           <input type="submit" name="pasajero2" value="Buscar" class="btn btn-color">
...
WHERE estudiantes.DOCUMENTO=:buscar AND  APROBADO=1 ORDER BY ID_CER";
...
if (isset($_POST['pasajero2'])){

$buscar = $_POST['buscarPasajero'];

$cadena = $_POST['buscarPasajero'];
    $numero = substr( $cadena, -5 );
    $codigo = (int)$numero;

    $sigla = substr( $cadena, 5, -5 );
...
INNER JOIN cursos ON cursos.ID = certificado_curso.ID_CURSO
INNER JOIN empresa ON empresa.ID_EMP = certificado_curso.ID_EMPRESA
WHERE SIGLA=:buscar AND  CODIGO=:cod AND APROBADO=1 ORDER BY ID_CER";

$registro=$conexion->prepare($consulta);

$registro->execute(array(":buscar"=>$sigla, ":cod"=>$codigo));
